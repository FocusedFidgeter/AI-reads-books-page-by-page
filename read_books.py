...
BOOK_KNOWLEDGE_DIR = KNOWLEDGE_DIR / BOOK_NAME
BOOK_SUMMARIES_DIR = SUMMARIES_DIR / BOOK_NAME
...
def setup_directories():
    ...
    for directory in [PDF_DIR, KNOWLEDGE_DIR, SUMMARIES_DIR, BOOK_KNOWLEDGE_DIR, BOOK_SUMMARIES_DIR]:
...
def save_summary(summary: str, is_final: bool = False):
    # Create book-specific summaries directory if it doesn't exist
    BOOK_SUMMARIES_DIR.mkdir(parents=True, exist_ok=True)
        
    # Create markdown file with proper naming
    if is_final:
        existing_summaries = list(BOOK_SUMMARIES_DIR.glob(f"{BOOK_NAME}_final_*.md"))
        next_number = len(existing_summaries) + 1
        summary_path = BOOK_SUMMARIES_DIR / f"{BOOK_NAME}_final_{next_number:03d}.md"
    else:
        existing_summaries = list(BOOK_SUMMARIES_DIR.glob(f"{BOOK_NAME}_interval_*.md"))
        next_number = len(existing_summaries) + 1
        summary_path = BOOK_SUMMARIES_DIR / f"{BOOK_NAME}_interval_{next_number:03d}.md"
